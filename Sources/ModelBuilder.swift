...
public struct ModelBuilder {
    ...
    /**
     Array of Any contains Dictionary

     - parameter array: an Array

     - returns: true if Array.Element is of Type Dictionary
     */
    func arrayContainsObjects(_ array : [Any]) -> Bool {
        var current = array
        while let first = current.first as? [Any] {
            current = first
        }

        if let _ = current.first as? [String:Any] {
            return true
        } else {
            return false
        }
     ...
     */
    func buildMultiDimentionalArrayOfObject(_ name:String, array:[Any]) -> (dataType:JSONDataType,content:[String:Any]?) {
        ...
        if let content = current.first as? [String:Any] {
     ...
     */
    public func buildModel(_ data:Any) -> ModelBuilder {
    ...
    /**
     ...
     - returns: A ModelBuilder containing the constructed Model
     */
    private func startWithArray(_ data:[Any]) -> ModelBuilder {
        var copy = self
    ...
    /**
     ...
     - returns: A ModelBuilder containing the constructed Model
     */
    private func startWithDictionary(_ data:[String:Any]) -> ModelBuilder {
        var copy = self
     ...
     */
    private func buildTypes(_ existingTypes:[JSONCustomType], name:String, fields:[String:Any]) -> [JSONCustomType] {
        ...
        for keyValuePair in fields {
            ...
            if let array = keyValuePair.1 as? [Any] where arrayContainsObjects(array) {
            ...
            // Is an object
            if let dict = keyValuePair.1 as? [String:Any] {
